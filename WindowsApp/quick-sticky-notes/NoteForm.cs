...
namespace quick_sticky_notes
{
    public partial class NoteForm : Form
    {
        ...
        private void richTextBox1_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                RTFDomDocument rtf = new RTFDomDocument();
                rtf.LoadRTFText(richTextBox1.SelectedRtf);
                if(rtf.Elements.Count==1&& rtf.Elements[0].Elements.Count==1&& rtf.Elements[0].Elements[0] is RTFDomImage image)
                {
                    using ( MemoryStream memoryStream = new MemoryStream(image.Data)){
                        try
                        {
                            Image image1 = Image.FromStream(memoryStream);
                            ShowImgForm.ShowForm(image1,MousePosition);
                        }
                        catch (Exception ex)
                        {
                            Debug.WriteLine(ex);
